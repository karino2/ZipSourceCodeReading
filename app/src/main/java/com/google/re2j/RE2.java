...
public class RE2 {
  ...
  static RE2 compileImpl(String expr, int mode, MatchKind matchKind, Options options)
    ...
    Slice prefixUTF8 = Slice.wrappedBuffer(prefixBuilder.toByteArray());
  ...
  Slice replaceAllFunc(Slice src, ReplaceFunc repl, int maxReplaces) {
    ...
    while (searchPos <= src.length()) {
      ...
      buf.write(src.getBuf(), lastMatchEnd, a[0] - lastMatchEnd);
      ...
      if (a[1] > lastMatchEnd || a[0] == 0) {
        try {
          buf.write(repl.replace(src.slice(a[0], a[1] - a[0])).getBuf());
        } catch (IOException e) {
    ...
    buf.write(src.getBuf(), lastMatchEnd, src.length() - lastMatchEnd);

    return Slice.wrappedBuffer(buf.toByteArray());
  }
